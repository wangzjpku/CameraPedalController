...
#define UART0_BAUD          9600  //最高支持921600
...
// PD0 PD1 PA12 // 黄色 蓝色 红色
  ...
  }else if ( ( GPD_0 == 1 ) && ( GPD_0 != keyboard[0] ) )
  {
		//GPD0 被抬起
		command[0] = 0xff;	
		command[1] = 0x01;  
  ...
  if ( ( GPA_12 == 0 ) && ( GPA_12 != keyboard[1] ) )
    ...
    // GPD1 被按下 
		...
		command[3] = 0x45;
		Send_Uart_Call(command );		
  }else if ( ( GPA_12 == 1 ) && ( GPA_12 != keyboard[1] ) )
  {
		//GPD1 被抬起
		command[0] = 0xff;	
		command[1] = 0x01;  
    ...
    // GPA12 被按下 
		...
		command[3] = 0x00;	
		Send_Uart_Call(command );		
  }else if ( ( GPD_1 == 1 ) && ( GPD_1 != keyboard[2] ) )
  {
		//GPA12 被抬起
		command[0] = 0xff;	
		command[1] = 0x00;  
